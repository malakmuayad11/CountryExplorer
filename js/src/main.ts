...
function getBorders(country: Country): string {
  ...
  country.borders.forEach((b: string) => {
    results += `<button class="borders cursor-pointer inline-block w-fit text-[0.65rem] text-center rounded-full border border-muted opacity-90 m-1 hover:opacity-50 p-0.5">${b}</button>`;
  });
  return results;
}
